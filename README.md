...
## Ver.0.64 暫定対応完了告知: 2026/06/29
バージョン EA_0.64f について、当プロジェクトへの取り込みが完了しました。  
ただし、06/29時点では暫定的な対応となっており、動作確認・表記揺れの解消その他が不十分な状態です。  
利用される方は上記についてご了承のうえ導入をお願いします。

**(経緯について)**  
06/19 に公開されたアップデートにて、公式が日本語訳の大幅な更新をしてくださいました。  
旧バージョンと比較すると、公式の日本語化率は倍近くまで向上しています。  
一方で、機械翻訳に起因すると思われる誤訳や表記揺れ・過去の翻訳との食い違いが多数発生しており、単純な改善とも言いづらい状況です。  
当プロジェクトでは引き続き、改善を図っていく予定です。

## 翻訳進捗
現在の翻訳完了率は全体の約 86.1% です。  
うち、当プロジェクトでの改善寄与率は約 22.7% です。  
参考: 公式版の日本語翻訳率は約 86.1% です。  
(本体のバージョン: EA_0.64g)

Jp-beta.csv はβブランチ用ですが、現在正常動作は保証できません。
